...
class PageTest(TestCase, WagtailTestUtils):
    def setUp(self):
        ...
        self.superuser = User.objects.create_superuser(
            username="superuser", email="superuser@gmail.com", password="superuser123"
        )
        self.superuser.save()
        # login superuser
    ...
    def check_edit_page_choices(self, page):
        ...
        widget_pos = 0
        for entry in response.context:
            if entry.get("widget", {}).get("name") == "dynamic_list":
                # debug: print(f"Found widget at position {widget_pos}")
                break
            widget_pos += 1

        choices = response.context[widget_pos]["widget"]["optgroups"]
        if page.__class__.__name__ == "PageWithDynamicList":
            self.assertEqual(
                [(x[1][0].get("value"), x[1][0].get("label")) for x in choices],
                dynamic_functions.DYNAMIC_LIST,
            )
        if page.__class__.__name__ == "PageStaticList":
            self.assertEqual(
                [(x[1][0].get("value"), x[1][0].get("label")) for x in choices],
                dynamic_functions.return_static_list(),
            )
